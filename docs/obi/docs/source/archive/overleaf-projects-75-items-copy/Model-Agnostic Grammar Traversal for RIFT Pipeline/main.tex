...
\SetKw{Default}{Default}
\renewcommand{\BlankLine}{\vspace{0.5\baselineskip}}
\usepackage{tikz}
\usepackage{pgfplots}
\pgfplotsset{compat=1.18}
\usepackage{listings}
\lstset{literate={ψ}{{$\psi$}}1}
\usepackage{xcolor}
\usepackage{hyperref}
\usepackage{cleveref}
\usepackage{booktabs}
...
\newcommand{\riftmatrix}[1]{\ensuremath{\mathbf{#1}}}
...
The input token stream is organized as a semantic matrix $\riftmatrix{M} \in \Sigma^{R \times C}$:
\begin{equation}
\riftmatrix{M} = \begin{bmatrix}
...
\KwIn{Semantic matrix $M \in \mathbb{R}^{R \times C}$, confidence threshold $\theta_{\min}$}
...
For any well-formed input matrix $\riftmatrix{M}$ and confidence threshold $\theta_{\min} > 0$, the traversal algorithm produces a complete parsing of all symbols with confidence $\geq \theta_{\min}$.
\end{theorem}
...
The intent resolution framework preserves semantic equivalence\label{def:semantic-equivalence} under isomorphic transformations.
\end{theorem}
...
% Bibliography section commented out until references.bib is available
